    ...
    <div className='playerIcon__container'>
      {players.map((player, index) => (
        <Label.Group key={index} className='playerIcon__group'>
          <Label 
            className='playerIcon__image' 
            key={index} 
            size='big' 
            active={index === playerTurnIndex}>
            <Image size='big' centered src={playerImages[player]} />
              {/* {index === playerTurnIndex ? <Icon name='paint brush'/> : ''} */}
            <div className='playerIcon__name'>{player}</div>
          </Label>
          <Label 
            active={index === playerTurnIndex}   className='playerIcon__score'>
            {playerPoints[player]}
          </Label>
        </Label.Group>
      ))}
